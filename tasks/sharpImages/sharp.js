...
const IMAGES = getFiles(`${raws}`) // Получение списка файлов из директории raws
...
async function proccesImages() {
  const ALL_IMAGES = await IMAGES // Ожидание завершения получения списка файлов
  console.log(ALL_IMAGES, 'ALL') // Вывод списка файлов в консоль
  for (let i = 0; i < ALL_IMAGES.length; i++) { // Итерация по списку файлов
    const path = ALL_IMAGES[i] // Получение пути к текущему файлу
    const fullPath = `./raws/${ALL_IMAGES[i]}` // Формирование полного пути к файлу
