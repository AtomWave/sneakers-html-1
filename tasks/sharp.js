...
const { raws, images } = config.paths // Извлечение свойств source, raws, images
...
const getImages = getFiles(`${raws}`) // Получение списка файлов из директории raws
let image = '' // Переменная для хранения экземпляра объекта sharp

const getPng = /.png/gm // Поиск файлов с расширением .png

// Асинхронная функция для обработки изображений
async function proccesImages() {
  const allImages = await getImages // Ожидание завершения получения списка файлов
  console.log(allImages, 'ALL') // Вывод списка файлов в консоль
  for (let i = 0; i < allImages.length; i++) { // Итерация по списку файлов
    const path = allImages[i] // Получение пути к текущему файлу
    const fullPath = `./raws/${allImages[i]}` // Формирование полного пути к файлу
